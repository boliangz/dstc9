...
class Analyzer:
    ...
    def comprehensive_analyze(self, sys_agent, model_name, total_dialog=100):
        ...
        for j in tqdm(range(total_dialog), desc="dialogue"):
            ...
            print("-"*10)
            print("dialogue %d", j+1)
            print('goal: ', sess.evaluator.goal, '\n')
            print('booked', sess.evaluator.booked, '\n')
            print('sys_da_array', sess.evaluator.sys_da_array, '\n')
            TP, FP, FN, bad_inform, reqt_not_inform, inform_not_reqt = sess.evaluator._inform_F1_goal(sess.evaluator._expand(sess.evaluator.goal), sess.evaluator.sys_da_array)
            print("TP, FP, FN: ", TP, FP, FN)
            print("bad_inform: ", bad_inform)
            print("reqt_not_inform: ", reqt_not_inform)
            print("inform_not_reqt: ", inform_not_reqt)
            print('domain_goals: ', sess.user_agent.policy.policy.goal.domain_goals, '\n')
            # print('usr_da_array ', sess.user_agent.get_out_da(),'\n')
            print('task complete: ', task_complete)
            print('task success: ', task_success)
            print('book rate: ', book_rate)
            print('inform precision/recall/f1: ', stats)
